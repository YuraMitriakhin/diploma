    ...
    <form method="post" action="/lessons/save" enctype="multipart/form-data">
      ...
      <div class="add-topic">
          ...
          <p><select class="choice-course" required name="courseId" >
                  <tbody>
                  <option value="${lesson.getCourseId().getCourseId()}" disabled>${lesson.getCourseId().getName()}</option>
                  </tbody>
                  <#list Courses as c>
                      <tbody>
         ...
         <p><input type="number" name="number" class="choose-number" onchange="l()" id="num" required value="${lesson.getNumber()}"></p>

        <p><@s.message "topic.name"/><span class="red">*</span></p>
        <p><textarea type="text" name="name" class="input-topic-name" maxlength="50" minlength="3" required>${lesson.getName()}</textarea> </p>
        <p><@s.message "topic.description"/><span class="red">*</span></p>
        <p><textarea type="text" name="description" class="input-short-description" maxlength="150" minlength="10" required >${lesson.getDescription()}</textarea> </p>
        <p><@s.message "topic.text"/><span class="red">*</span></p>
        <p ><textarea type="text" name="text" class="input-description" minlength="100" required >${lesson.getText()}</textarea></p>
        <p><@s.message "topic.add.photo"/></p>
          <div id="file-input">
              <#list lesson.getFiles() as f>
                  <p><input type="file" name="files" onchange="checkFileSize(this)" value="${f.getFile()}">
                  <input type="text" name="descriptionToFiles" value="${f.getDescription()}"></p>
              </#list>
          </div>
          ...
          <div id="links">
              <#list lesson.getLinks() as l>
                  <p><input class="input-link" type="url" name="links" value="${l.getLink()}"></p>
              </#list>
          </div>
